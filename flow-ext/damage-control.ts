...
export default function (pi: ExtensionAPI) {
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		const rulesPath = path.join(os.homedir(), ".pi", "agent", "damage-control-rules.yaml");
		if (fs.existsSync(rulesPath)) {
			...
			ctx.ui.notify("🛡️ Damage-Control: No rules found at ~/.pi/agent/damage-control-rules.yaml");
		}
